...
* **Master Agent (总控智能体)**：位于 `supervisor.py`。负责解析输入的招标文件，利用图谱和标签拆解项目核心元数据（如上限控制价、硬性资质、特殊痛点工况），并分发任务。
...
sequenceDiagram
    ...
    LangGraph->>Nodes: Master Agent 解析需求，唤醒 Workers
    Nodes->>Nodes: Tech Worker (流水线) 检索工艺，生成文案
    Nodes->>PG: [DB First] 生成文案立即落库，绑定溯源证据
