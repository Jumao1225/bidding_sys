...
class EvaluationSchema(BaseModel):
    ...
    hard_service_requirements: dict[str, Any] = Field(
        default_factory=dict,
        description="提取到的硬性服务/质保约束（如：{'质保期': '3年', '响应时间': '2小时', '违约扣款': '每超时1天扣0.5%'}）"
    )
...
5. **硬性售后约束剥离 (`hard_service_requirements`)**：仔细寻找任何涉及“响应时间”、“质保期/缺陷责任期”、“罚款/违约金”的要求。无论这些要求是否在评分表里，都必须将它们强制抽离并以字典形式（如 `{"质保期": "3年"}`）独立写入，确保服务兜底不丢失。
